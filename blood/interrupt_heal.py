...
``update_heal_timer``(推进倒计时)与 ``update_hp``(回血/掉血)共同工作:
    ...
    - 正常流程:``heal_timer`` 从 ``heal_interval`` 开始逐帧递减,
      减到 0 后触发一次回血并重置。
    - 受击流程:命中判定成立、对玩家造成伤害时,调用 ``interrupt_heal``,
...
def interrupt_heal(player_state):
    ...
    Args:
        player_state: 单个玩家的状态 ``dict``,需包含以下键:
            - ``heal_timer``: 当前回血倒计时(秒或帧),随时间递减。
            - ``heal_interval``: 满回血间隔,即受击后需等待多久才能再次回血。
    ...
    player_state["heal_timer"] = player_state["heal_interval"]
